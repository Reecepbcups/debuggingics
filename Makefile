...
HTTPS_GIT := github.com/rollchains/myproject.git
...
ictest-ics-basic:
	@echo "Running Interchain Security e2e test"
	@cd interchaintest && go test -race -v -run TestICSBasic .
...
set-testnet-configs:
	rolld config set client chain-id chainid-1
...
sh-testnet: mod-tidy
	CHAIN_ID="local-1" BLOCK_TIME="1000ms" CLEAN=true sh scripts/test_ics_node.sh
...
help:
	...
	@echo "  testnet             : Local devnet with IBC to a local CosmosHub"
	@echo "  testnet-basic       : Standalone devnet"
